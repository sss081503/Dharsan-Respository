...
def groupSum5(start, nums, target):
    if start >= len(nums):
        return target == 0
    elif (nums[start] % 5 == 0) and (start + 1 < len(nums)):
        if nums[start + 1] == 1:
...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        return target == 0
    elif (start < len(nums)-1) and nums[i] == nums[i+1]:
        count = 0
        value = 0
        while 
        if count >= 1:
            return groupSumClump(start + count, nums, target - count * nums[count]) or groupSumClump(start + count, nums, target)
        else:
            return groupSumClump(start + 1, nums, target - nums[count]) or groupSumClump(start + 1, nums, target)
